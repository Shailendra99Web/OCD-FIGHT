  ...
  return (
    <div className='m-3 min-h-[80vh]'>
      ...
      <div className='flex text-center justify-center items-center'>
        <button className='px-4 py-2 rounded-lg hover:text-slate-400 dark:hover:bg-slate-300 hover:bg-slate-100'>
          <Link href={`/history/${Number(params.var[0]) - 1}`} className='font-bold'>&larr;</Link>
        </button><h1 className='text-xl text-center p-4 inline-block w-60'>{getMonthName(params.var[0])}&#44; 2024</h1><button className='px-4 py-2 rounded-lg hover:text-slate-400 dark:hover:bg-slate-300 hover:bg-slate-100'><Link href={(Number(params.var[0]) + 1 <= new Date().getMonth() + 1) ? `/history/${Number(params.var[0]) + 1}` : `/history/${params.var[0]}`} className='font-bold '>&rarr;</Link>
        </button>
      </div>
      ...
      <div className='overflow-auto'>
        ...
        <table className="m-auto sm:w-3/5 table-auto border-collapse border border-slate-500">
          ...
          <tbody className='bg-slate-100 dark:bg-transparent'>
            ...
            {/* <tr>
            <td rowSpan='2' className='border border-gray-300 dark:border-slate-600 p-2 dark:text-slate-300 text-slate-500'>25 Oct</td>
            <td className='border border-gray-300 dark:border-slate-600 p-2 text-blue-500'>COM: 00</td>
          ...
          <tr>
            <td className='border border-gray-300 dark:border-slate-600 p-2 text-green-500'>RUM: 00:00</td>
            ...
            {monthHistory && monthHistory.length > 0 ? monthHistory.map((day, index) => {
              ...
              return <>
                <tr>
                  ...
                  <td className='border border-gray-300 dark:border-slate-600 p-2 text-blue-500'>{totalCompulsions}</td>
                  {dayData.map((count, index) => {
